            ...
            <div key={group.rank.id} className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {group.members.map((member, i) => (
                  ...
                  >
                    <Link 
                      to={`/player/${encodeURIComponent(member.ign)}`}
                      className="block glass rounded-3xl p-6 text-center group hover:border-purple-500/50 transition-all duration-500 cursor-pointer"
                    >
                      ...
                      <h3 className="text-xl font-bold text-white mb-1 group-hover:text-purple-300 transition-colors">{member.ign}</h3>
                      <p className="text-purple-400 text-sm font-bold uppercase tracking-widest">{group.rank.name}</p>
                      <span className="inline-block mt-3 text-xs text-slate-500 font-semibold group-hover:text-purple-400 transition-colors">View Player Profile →</span>
                    </Link>
                  </motion.div>
                ))}
              </div>
