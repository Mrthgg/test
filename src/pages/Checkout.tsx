...
export default function Checkout() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8 min-w-0 w-full">
      <div className="text-center space-y-3">
        <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight">MANUAL <span className="text-purple-500">CHECKOUT</span></h1>
        <p className="text-slate-400 text-sm md:text-base max-w-2xl mx-auto">
          Complete your payment using your preferred method and provide the transaction details below.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 min-w-0">
        <div className="space-y-6 min-w-0">
          <div className="glass p-5 md:p-8 rounded-3xl space-y-6 min-w-0 w-full">
            <h2 className="text-xl font-bold text-white flex items-center space-x-2">
              <CreditCard className="w-6 h-6 text-purple-500 flex-shrink-0" />
              <span>Payment Details</span>
            </h2>
            
            <form onSubmit={handleSubmit} className="space-y-5 min-w-0 w-full">
              {error && (
                <div className="bg-red-500/10 border border-red-500/50 text-red-500 px-4 py-3 rounded-xl flex items-center space-x-3 text-sm break-words min-w-0">
              ...
              <div className="space-y-2 min-w-0">
                ...
                <div className="relative min-w-0">
                  ...
                  <input
                    ...
                    className="w-full min-w-0 bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all text-sm md:text-base"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2 min-w-0">
                ...
                <select
                  ...
                  className="w-full min-w-0 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all text-sm md:text-base cursor-pointer"
                >
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold py-3.5 md:py-4 px-4 rounded-xl transition-all neon-glow-purple disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base cursor-pointer"
              >
        ...
        <aside className="space-y-6 md:space-y-8 min-w-0">
          <div className="glass p-5 md:p-8 rounded-3xl space-y-6 min-w-0">
            <h2 className="text-xl font-bold text-white">Order Summary</h2>
            <div className="space-y-4 min-w-0">
              {cart.map((item) => (
                <div key={item.productId} className="flex justify-between items-center text-sm min-w-0 gap-2">
                  <span className="text-slate-400 truncate">{item.name} x{item.quantity}</span>
                  <span className="text-white font-medium flex-shrink-0">{item.price * item.quantity} BDT</span>
                </div>
              ))}
