  ...
  return (
    <div className="min-h-screen flex flex-col">
      <nav className="sticky top-0 z-50 glass border-b border-slate-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:block">
              <div className="ml-6 xl:ml-10 flex items-center space-x-1 xl:space-x-3">
                {navLinks.map((link) => (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={cn(
                      "px-2.5 py-1.5 rounded-md text-xs xl:text-sm font-medium transition-colors whitespace-nowrap",
            ...
            <div className="hidden lg:flex items-center space-x-3">
              ...
              <Link to="/cart" className="relative p-2 text-slate-400 hover:text-white transition-colors" title="Cart">
              ...
              {user ? (
                <Link 
                  to="/account" 
                  className="flex items-center space-x-2 pl-2 pr-3 py-1 bg-slate-900 border border-slate-800 hover:border-purple-500/50 rounded-xl transition-all"
                  title="My Account"
                >
                  <img 
                    src={`https://mc-heads.net/avatar/${profile?.display_name || 'steve'}/24`} 
                    alt="Head" 
                  ...
                  />
                  <span className="text-xs font-bold text-white max-w-[100px] truncate">{profile?.display_name || user.email?.split('@')[0]}</span>
                </Link>
              ) : (
                <Link
                  to="/login"
                  className="flex items-center space-x-1.5 px-3 py-1.5 bg-purple-600/10 hover:bg-purple-600/20 border border-purple-500/30 text-purple-300 rounded-xl text-xs font-bold transition-all"
                >
            ...
            <div className="lg:hidden flex items-center space-x-2">
              ...
              <Link to={user ? "/account" : "/login"} className="p-2 text-slate-400 hover:text-white transition-colors" title="Account">
        ...
        {/* Mobile Nav */}
        {isMenuOpen && (
          <div className="lg:hidden glass border-t border-slate-800/50">
